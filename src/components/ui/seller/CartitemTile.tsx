...
import { CartItem } from '@/app/(tabs)/profile/write-to-seller';
...
function CartItemTile(props: CartItem) {
  ...
  const imagURL = isValidHttpUrl(image) ? { uri: image } : image;
  ...
  return (
    ...
    >
      <Image
        source={imagURL}
        style={{ minHeight: 120, minWidth: 120, borderRadius: 10 }}
      />
      <View
        style={{
          display: 'flex',
      ...
      >
        ...
        >
          ...
          <SvgIcons
            name={CUSTOM_ICON_REF.Storefront}
            baseStyle={{ width: 30, height: 30 }}
          />
        </View>
        <View
          style={{
        ...
        >
          ...
          >
            ...
            ></View>
            <Text
              style={{ paddingLeft: 10, color: '#666666', fontWeight: 'bold' }}
            >
              {variant.size
                ? variant.size + ' /' + quantity + ' шт.'
                : quantity + 'шт.'}
            </Text>
          </View>
          <Text style={{ fontWeight: 'bold', color: '#170F2B' }}>
            {price}
            {currency}
          </Text>
        </View>
